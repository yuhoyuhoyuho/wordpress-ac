...
install_wordpress_with_wp_cli() {
    ...
    docker exec --user www-data "$CONTAINER_NAME" wp user create \
        "$WP_ADMIN_USER" \
        "$WP_ADMIN_EMAIL" \
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if docker exec "${site_name}-wp" wp --allow-root user get "$customer_username" --field=roles 2>/dev/null | grep -q "shopmanager"; then
                log "✅ 客户用户角色验证成功: $customer_username -> shopmanager"
            fi
