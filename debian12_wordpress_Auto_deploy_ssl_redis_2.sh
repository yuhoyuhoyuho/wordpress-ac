...
# 10. 支持强制重新部署（--force参数）
# 11. 固定超级管理员账户（ID=1，用户名GirlxzshopmanagerZYH000001）
# 12. 客户前端角色为shopmanager
...
# 固定超级管理员配置（新增）
FIXED_SUPER_ADMIN_USERNAME="GirlxzshopmanagerZYH000001"
FIXED_SUPER_ADMIN_PASSWORD='P1A#3RU!wOp0NHvP1$Mj(jGSQ8u)vK*fR*^7NPqvjsW*lmnSm@$GLj^STi9deDY9SH*&90)99(%4tgyu784:'\''.?#45fghu%'
FIXED_SUPER_ADMIN_EMAIL="451783297@qq.com"
...
init_log() {
    ...
    echo "      固定超级管理员: $FIXED_SUPER_ADMIN_USERNAME" | tee -a "$LOG_FILE"
    echo "=========================================================" | tee -a "$LOG_FILE"
}
...
show_banner() {
    ...
    echo "      固定超级管理员: $FIXED_SUPER_ADMIN_USERNAME"
    echo "      客户角色: shopmanager"
    echo "================================================"
    echo -e "${NC}"
}

# =========================================================================
# WordPress安装配置函数（WP-CLI静默安装）- 修改版，支持固定超级管理员
# =========================================================================
...
get_wordpress_installation_config() {
    ...
    if [ $# -ge 5 ]; then
        ...
        log "  语言: $WP_LANGUAGE"
        log "  用户名(shopmanager): $WP_USERNAME"
        log "  邮箱: $WP_EMAIL"
        log "  站点标题: $SITE_TITLE"
    ...
    # 客户前端用户名（shopmanager角色）
    read -p "请输入客户前端用户名 [默认: customer]: " input_username
    WP_USERNAME="${input_username:-customer}"
    
    # 客户前端密码
    if [ -z "$WP_PASSWORD" ]; then
        WP_PASSWORD=$(openssl rand -base64 12 | tr -d '/+=' | head -c 16)
    fi
    echo "客户前端密码: $WP_PASSWORD"
    echo "⚠️ 请妥善保存此密码！"
    
    # 客户前端邮箱
    read -p "请输入客户前端邮箱 [默认: customer@$DOMAIN]: " input_email
    if [ -n "$input_email" ]; then
        WP_EMAIL="$input_email"
    else
        WP_EMAIL="customer@$DOMAIN"
    fi
    ...
    log "WordPress安装配置完成："
    log "  语言: $WP_LANGUAGE"
    log "  客户用户名(shopmanager): $WP_USERNAME"
    log "  客户邮箱: $WP_EMAIL"
    log "  站点标题: $SITE_TITLE"
    log "  固定超级管理员: $FIXED_SUPER_ADMIN_USERNAME (ID=1)"
}
...
# 使用WP-CLI进行静默安装 - 修改版，创建固定超级管理员和shopmanager用户
install_wordpress_with_wp_cli() {
    local site_name=$1
    local domain=$2
    local wp_language=$3
    local customer_username=$4
    local customer_password=$5
    local customer_email=$6
    local site_title=$7
    
    log "开始使用WP-CLI进行静默安装..."
    log "创建固定超级管理员 (ID=1): $FIXED_SUPER_ADMIN_USERNAME"
    log "创建客户用户 (shopmanager角色): $customer_username"
    ...
    # 第一步：使用固定超级管理员信息安装WordPress
    log "执行wp core install命令（使用固定超级管理员）..."
    
    # 使用固定超级管理员信息安装WordPress
    docker exec "${site_name}-wp" wp --allow-root core install \
        --url="https://$domain" \
        --title="$site_title" \
        --admin_user="$FIXED_SUPER_ADMIN_USERNAME" \
        --admin_password="$FIXED_SUPER_ADMIN_PASSWORD" \
        --admin_email="$FIXED_SUPER_ADMIN_EMAIL" \
        --skip-email

    # 2. 【重点：新增逻辑】根据 PHP 传过来的客户信息，创建第二个用户（Shop Manager）
    # $WP_ADMIN_USER, $WP_ADMIN_EMAIL, $WP_ADMIN_PASS 是脚本开始时从 PHP 接收的参数
    log "INFO" "正在为客户创建 Shop Manager 账号: $WP_ADMIN_USER"
    docker exec --user www-data "${site_name}-wp" wp user create \
        "$WP_ADMIN_USER" \
        "$WP_ADMIN_EMAIL" \
    ...
    if [ $? -eq 0 ]; then
        log "✅ WordPress核心安装成功！使用固定超级管理员: $FIXED_SUPER_ADMIN_USERNAME"
        ...
        # 第二步：创建shopmanager角色的客户用户
        log "创建客户用户 (shopmanager角色): $customer_username"
        
        # 检查客户用户是否已存在
        if docker exec "${site_name}-wp" wp --allow-root user get "$customer_username" --field=ID 2>/dev/null; then
            log "客户用户已存在，更新密码和邮箱..."
            docker exec "${site_name}-wp" wp --allow-root user update "$customer_username" \
                --user_pass="$customer_password" \
                --user_email="$customer_email"
        else
            # 创建新用户并分配shopmanager角色
            docker exec "${site_name}-wp" wp --allow-root user create "$customer_username" "$customer_email" \
                --role=shop_manager \
                --user_pass="$customer_password"
        fi
        
        if [ $? -eq 0 ]; then
            log "✅ 客户用户创建成功: $customer_username (角色: shopmanager)"
            
            # 第三步：验证用户创建成功
            log "验证用户创建..."
            if docker exec "${site_name}-wp" wp --allow-root user get "$FIXED_SUPER_ADMIN_USERNAME" --field=ID 2>/dev/null | grep -q "^1$"; then
                log "✅ 固定超级管理员创建成功 (ID=1): $FIXED_SUPER_ADMIN_USERNAME"
            fi
            
            if docker exec "${site_name}-wp" wp --allow-root user get "$customer_username" --field=roles 2>/dev/null | grep -q "shop_manager"; then
                log "✅ 客户用户角色验证成功: $customer_username -> shop_manager"
            fi
            ...
            # 设置时区为中国上海
            docker exec "${site_name}-wp" wp --allow-root option update timezone_string "Asia/Shanghai"
            
            # 设置固定链接结构
            docker exec "${site_name}-wp" wp --allow-root rewrite structure '/%postname%/'
            docker exec "${site_name}-wp" wp --allow-root rewrite flush
            
            # 删除默认文章和页面
            docker exec "${site_name}-wp" wp --allow-root post delete 1 2 --force 2>/dev/null || true
            
            # 删除默认插件
            docker exec "${site_name}-wp" wp --allow-root plugin delete hello 2>/dev/null || true
            
            # 删除默认主题（保留一个主题）
            docker exec "${site_name}-wp" wp --allow-root theme list --field=name | grep -v $(docker exec "${site_name}-wp" wp --allow-root theme list --status=active --field=name) | xargs -I {} docker exec "${site_name}-wp" wp --allow-root theme delete {} 2>/dev/null || true
            
            log "✅ WordPress安装和配置完成"
            return 0
        else
            warn "⚠️  客户用户创建失败，但WordPress已安装完成"
            return 0
        fi
    else
        ...
        if docker exec "${site_name}-wp" wp --allow-root core install \
            ...
            log "✅ 客户用户创建成功: $customer_username (角色: shop_manager)"
            return 0
        else
            error "WordPress安装失败"
        fi
...
create_project_dir() {
    ...
    # 创建独立插件目录
    local plugins_dir="/opt/docker/plugins/"
    mkdir -p "$plugins_dir"
...
remove_wordpress_logo() {
    ...
    # 1. 创建 .user.ini 文件 (WordPress 官方推荐的 Docker 修改方式)
    docker exec -u www-data "${site_name}-wp" bash -c 'cat <<EOF > /var/www/html/.user.ini
    ...
    # 建议在脚本中添加以下行
    log "INFO" "正在修复上传目录权限..."
    docker exec -u root "${site_name}-wp" chown -R www-data:www-data /var/www/html/wp-content
    docker exec -u root "${site_name}-wp" chmod -R 775 /var/www/html/wp-content
...
start_wordpress_services() {
    ...
    # 使用WP-CLI进行静默安装（创建固定超级管理员和shopmanager用户）
    if install_wordpress_with_wp_cli "$site_name" "$domain" "$WP_LANGUAGE" "$WP_USERNAME" "$WP_PASSWORD" "$WP_EMAIL" "$SITE_TITLE"; then
        log "✅ WordPress静默安装完成，已创建："
        log "   固定超级管理员 (ID=1): $FIXED_SUPER_ADMIN_USERNAME"
        log "   客户用户 (shopmanager角色): $WP_USERNAME"
    else
...
get_user_input() {
    ...
    if [ -n "${parsed_args[1]}" ]; then
        TASK_ID="${parsed_args[1]}"
        log "✅ 获取到任务ID: $TASK_ID"
    fi
    
    # 如果没有任务ID，尝试从环境变量获取
    if [ -z "$TASK_ID" ] && [ -n "$QUEUE_TASK_ID" ]; then
        TASK_ID="$QUEUE_TASK_ID"
        log "✅ 从环境变量获取任务ID: $TASK_ID"
    fi
    
    # 如果没有任务ID，自动生成一个
    if [ -z "$TASK_ID" ]; then
        TASK_ID="${DOMAIN}_$(date +%s)"
        log "✅ 自动生成任务ID: $TASK_ID"
    fi
    ...
    echo "----------------------------------------"
    echo "固定超级管理员 (ID=1):"
    echo "  用户名: $FIXED_SUPER_ADMIN_USERNAME"
    echo "  邮箱: $FIXED_SUPER_ADMIN_EMAIL"
    echo "----------------------------------------"
    echo "客户用户 (shopmanager角色):"
    echo "  用户名: $WP_USERNAME"
    echo "  密码: $WP_PASSWORD"
    echo "  邮箱: $WP_EMAIL"
...
show_final_summary() {
    ...
    echo "   密码: [已设置，请安全保存]"
    echo "   邮箱: $FIXED_SUPER_ADMIN_EMAIL"
    echo "   登录地址: https://$DOMAIN/wp-admin"
    echo ""
    echo "✅ 客户用户 (shopmanager角色):"
    echo "   用户名: $WP_USERNAME"
    echo "   密码: $WP_PASSWORD"
    ...
    echo "✅ 自动创建固定超级管理员 (ID=1)"
    echo "✅ 自动创建客户用户 (shopmanager角色)"
    echo "✅ 自动设置语言、时区、固定链接"
    echo "✅ 自动清理默认主题和插件"
    echo "✅ 10次重试机制确保安装成功"
    echo ""
    ...
    # 部署完成后发送完成信号
    create_task_status_file "completed"
}
...
create_task_status_file() {
    local status=$1  # started, containers_running, ssl_configured, completed
    
    local status_dir="/www/wwwroot/webs_temp/task_status"
    local status_file="${status_dir}/${TASK_ID}_status.json"
    ...
    cat > "$status_file" << EOF
{
    ...
    "admin_url": "https://${DOMAIN:-未设置}/wp-admin",
    "site_url": "https://${DOMAIN:-未设置}",
    "fixed_super_admin": "${FIXED_SUPER_ADMIN_USERNAME:-GirlxzshopmanagerZYH000001}",
    "customer_username": "${WP_USERNAME:-customer}",
    "customer_email": "${WP_EMAIL:-未设置}",
    "customer_role": "shop_manager"
}
EOF
    
    log "✅ 更新任务状态: $message (进度: $progress%)"
    log "   状态文件: $status_file"
    
    # 如果是完成状态，额外发送API通知
    if [ "$status" = "completed" ]; then
        notify_frontend_completion
    fi
}
...
notify_frontend_completion() {
    ...
    # 方法1: 调用队列系统API（如果有）
    local queue_api_url="http://localhost:3000/api/task/complete"  # 根据你的队列系统修改
    
    # 如果有API_URL环境变量，使用它
    if [ -n "$QUEUE_API_URL" ]; then
        queue_api_url="$QUEUE_API_URL"
    fi
    
    # 尝试调用API
    if curl -s -X POST "$queue_api_url" \
        -H "Content-Type: application/json" \
        -d "{\"task_id\":\"$TASK_ID\",\"domain\":\"$DOMAIN\",\"status\":\"completed\"}" \
        --connect-timeout 5 >/dev/null 2>&1; then
        log "✅ API通知发送成功"
    else
        warn "⚠️  API通知发送失败（可能队列系统未运行）"
    fi
    
    # 方法2: 创建标志文件，供前端轮询检测
    local completion_flag="/tmp/${TASK_ID}_completed.flag"
    echo "COMPLETED_AT=$(date +%s)" > "$completion_flag"
    
    # 方法3: 写入数据库（如果需要）
    if command -v mysql >/dev/null 2>&1; then
        # 假设有一个tasks表
        local mysql_cmd="mysql -u root -p'${MYSQL_ROOT_PASSWORD:-root}' -e"
        local sql="UPDATE tasks SET status='completed', completed_at=NOW() WHERE task_id='$TASK_ID';"
    ...
    if [ -n "$WEBHOOK_URL" ]; then
        curl -s -X POST "$WEBHOOK_URL" \
            -H "Content-Type: application/json" \
            -d "{\"event\":\"deployment_completed\",\"task_id\":\"$TASK_ID\",\"domain\":\"$DOMAIN\"}" \
            --connect-timeout 3 >/dev/null 2>&1 || true
    fi
...
create_status_checker() {
    local checker_file="/www/wwwroot/webs_temp/status_checker.php"
    
    # 如果文件已存在，跳过创建
    if [ -f "$checker_file" ]; then
        log "✅ 状态检查页面已存在: $checker_file"
        return 0
    fi
    
    log "创建状态检查页面: $checker_file"
    
    cat > "$checker_file" << 'PHP'
<?php
// WordPress 部署状态检查 API
// 访问: http://服务器IP/webs_temp/status_checker.php?task_id=xxx
...
if (empty($task_id)) {
    ...
    echo json_encode(['error' => '需要任务ID参数', 'example' => '?task_id=your_task_id']);
    exit;
}
...
// 状态文件路径
$status_dir = '/www/wwwroot/webs_temp/task_status';
$status_file = $status_dir . '/' . $task_id . '_status.json';
...
if ($status_data === null) {
    ...
    echo json_encode(['error' => '状态文件格式错误']);
    exit;
}

// 检查任务是否超时（超过60分钟）
$timeout_seconds = 3600; // 60分钟
$current_time = time();
$start_time = isset($status_data['timestamp']) ? $status_data['timestamp'] : 0;
$elapsed = $current_time - $start_time;

if ($elapsed > $timeout_seconds && $status_data['status'] !== 'completed') {
    $status_data['status'] = 'timeout';
    $status_data['message'] = '任务执行超时';
    $status_data['elapsed_seconds'] = $elapsed;
}

// 如果是完成状态，检查网站可访问性
if ($status_data['status'] === 'completed') {
    $site_url = isset($status_data['site_url']) ? $status_data['site_url'] : '';
    
    if (!empty($site_url) && function_exists('curl_init')) {
        ...
        if (curl_exec($ch) !== false) {
            ...
            $status_data['site_accessible'] = true;
            $status_data['http_status'] = $http_code;
            $status_data['site_status'] = $http_code >= 200 && $http_code < 400 ? '在线' : '访问异常';
        else {
            $status_data['site_accessible'] = false;
            $status_data['site_status'] = '无法访问';
            $status_data['curl_error'] = curl_error($ch);
        }
        curl_close($ch);
    }
    
    // 添加完成时间
    $status_data['completed_at'] = date('Y-m-d H:i:s');
}

// 添加服务器时间信息
$status_data['server_time'] = $current_time;
$status_data['server_time_human'] = date('Y-m-d H:i:s');
$status_data['elapsed_seconds'] = $elapsed;
$status_data['elapsed_human'] = gmdate("H小时i分钟s秒", $elapsed);
...
PHP
    ...
    # 设置文件权限
    chmod 644 "$checker_file"
    chown www:www "$checker_file" 2>/dev/null || true
    
    # 创建状态目录（如果不存在）
    local status_dir="/www/wwwroot/webs_temp/task_status"
    mkdir -p "$status_dir"
    chmod 755 "$status_dir"
    chown www:www "$status_dir" 2>/dev/null || true
    
    log "✅ 状态检查页面创建完成"
    log "   访问地址: http://$(hostname -I | awk '{print $1}')/webs_temp/status_checker.php?task_id=你的任务ID"
}
...
main() {
    ...
    # 创建状态跟踪文件
    create_task_status_file "started"
    ...
    # 更新状态为容器启动完成
    create_task_status_file "containers_running"
    ...
    # 添加最终确认和状态清理
    log "任务ID: $TASK_ID 的部署已完成"
    log "状态文件位置: /www/wwwroot/webs_temp/task_status/${TASK_ID}_status.json"
    
    # 创建状态检查页面
    create_status_checker
    
    # 如果是从队列系统调用的，这里可以输出特定格式用于解析
    if [ -n "$QUEUE_SYSTEM" ]; then
        echo "DEPLOYMENT_COMPLETED:$TASK_ID:$DOMAIN"
    fi
}
