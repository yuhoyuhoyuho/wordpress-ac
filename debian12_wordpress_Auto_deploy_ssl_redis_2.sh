...
# WordPress 完整自动化部署脚本 - 全能融合版（增强SSL版）
# 融合功能：
...
init_log() {
    echo "=========================================================" > "$LOG_FILE"
    echo "      WordPress 全能融合部署日志 (融合版)" | tee -a "$LOG_FILE"
...
# 获取可用的IP地址 - 修复IP地址格式问题
get_available_ip() {
    local base_ip=$1
    local ip_type=$2  # web, db, redis
    
    if [ -z "$base_ip" ]; then
        # 生成基于站点名称的随机IP段，避免与Docker默认网络冲突
        local site_hash=$(echo "$SITE_NAME" | md5sum | cut -d' ' -f1 | head -c 8)
        local part1=$((0x${site_hash:0:2} % 50 + 10))  # 10-59，避免172.16-172.31的Docker默认范围
        local part2=$((0x${site_hash:2:2} % 240 + 16))  # 16-255
        local part3=$((0x${site_hash:4:2} % 240 + 16))  # 16-255
        
        base_ip="10.$part1.$part2"
    fi
    
    # 根据类型分配最后一个字节 - 修复：base_ip是3部分，加上第4部分
    case $ip_type in
        "web") echo "${base_ip}.10" ;;
        "db") echo "${base_ip}.20" ;;
        "redis") echo "${base_ip}.30" ;;
        *) echo "${base_ip}.40" ;;
    esac
}
...
allocate_resources() {
    ...
    # 检查是否已分配
    if check_domain_deployed "$domain"; then
        release_lock
        error "域名 $domain 已部署或正在部署中"
    fi
...
wait_for_redis() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        elif [ "$container_name" = "not_found" ]; then
...
# =========================================================================
# WordPress 站点 URL 自动配置（修复版重要功能）
# =========================================================================

# 配置 WordPress 站点 URL
configure_wordpress_url() {
    local site_name=$1
    local domain=$2
    
    log "配置 WordPress 站点 URL..."
    
    # 等待 WordPress 容器完全启动
    log "等待 WordPress 容器启动..."
    for i in {1..30}; do
        if docker exec "${site_name}-wp" curl -f http://localhost/wp-admin/install.php > /dev/null 2>&1; then
            log "WordPress 已启动，开始配置站点 URL"
            break
        fi
        if [ $i -eq 30 ]; then
            warn "WordPress 启动较慢，将在后台继续配置，请稍后访问网站"
            return 0
        fi
        sleep 2
    done
...
if (!$wp_loaded) {
    ...
    if ($mysqli->connect_error) {
        ...
        echo "站点 URL 已通过直接数据库连接更新为: {$home_url}\n";
        $stmt->close();
        $mysqli->close();
    }
} else {
    ...
    echo "站点 URL 已通过WordPress函数更新为: https://" . $_SERVER['HTTP_HOST'] . "\n";
}
?>
EOF
    
    # 复制到容器并执行
    docker cp update_urls.php "${site_name}-wp:/var/www/html/update_urls.php"
    
    # 等待WordPress完全初始化
    sleep 10
    
    # 执行更新脚本
    if docker exec "${site_name}-wp" php /var/www/html/update_urls.php; then
        log "✅ WordPress 站点 URL 配置完成"
    else
        warn "⚠️  站点URL配置执行失败，但将继续部署"
    fi


    # ==========================================================
    # 核心增强：物理删除/替换 WordPress 核心 Logo（处理 install.php）
    # ==========================================================
   
    # 2. 物理替换：将 Logo 图片直接替换为 1x1 像素的透明图片（这是最彻底的方法）
    # 我们生成一个透明的 GIF 并覆盖原始的 SVG 和 PNG
    local transparent_pixel="R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
    docker exec "${site_name}-wp" bash -c "echo '$transparent_pixel' | base64 -d > /var/www/html/wp-admin/images/wordpress-logo.svg"
    docker exec "${site_name}-wp" bash -c "echo '$transparent_pixel' | base64 -d > /var/www/html/wp-admin/images/w-logo-blue.png"
    docker exec "${site_name}-wp" bash -c "echo '$transparent_pixel' | base64 -d > /var/www/html/wp-admin/images/wordpress-logo.png"

    # 3. 强制 CSS 屏蔽：修改安装页专用的 CSS 文件
    # 彻底隐藏包含 logo 类的链接，并重置背景
    docker exec "${site_name}-wp" bash -c "echo '.wp-core-ui .mu-item:before, .wp-core-ui h1 a, #logo { display: none !important; background-image: none !important; }' >> /var/www/html/wp-admin/css/install.css"
    ...
    log "✅ WordPress 安装页 Logo 已物理移除"
    # ==========================================================

    
    # 清理临时文件
    rm -f update_urls.php
    docker exec "${site_name}-wp" rm -f /var/www/html/update_urls.php 2>/dev/null || true
    
    log "✅ WordPress 站点 URL 配置流程完成"
}
...
start_wordpress_services() {
    ...
    if $docker_compose_cmd up -d; then
        log "✅ WordPress 服务启动命令执行成功"
    else
        error "❌ WordPress 服务启动失败"
    fi
    ...
    # 配置 WordPress 站点 URL（新增的重要功能）
    configure_wordpress_url "$site_name" "$domain"
...
create_temp_http_config() {
    ...
    # 确保配置目录存在
    if [ ! -d "$conf_path" ]; then
        log "Nginx 配置目录 $conf_path 不存在，请检查宝塔面板是否已安装 Nginx。"
        return 1
    fi
    ...
    cat > "$conf_file" << EOF
# WordPress Docker 站点临时配置（等待SSL证书）
# 生成时间: $(date)
# 域名: ${domain}
...
server {
    ...
    # ACME 证书验证目录 - 关键：允许访问.well-known目录进行SSL证书验证
...
# =========================================================================
# Cloudflare DNS API 函数（内网+FRP环境专用）
# =========================================================================
...
apply_ssl_with_cf_dns() {
    ...
    if /root/.acme.sh/acme.sh --issue --dns dns_cf \
        ...
        --keylength ec-256 \
        --force; then
        log "✅ SSL 证书申请成功（Cloudflare DNS验证）"
...
get_user_input() {
    echo
    log "请输入 WordPress 站点配置"
    echo "================================================"

    # 关键修复：优先从命令行参数 $1 获取域名
    local input_arg="$1"
    if [ -n "$input_arg" ]; then
        DOMAIN="$input_arg"
    ...
    # 第一步：原子性资源分配（避免多用户冲突）
    allocate_resources "$DOMAIN" "$SITE_NAME"
    ...
    echo "站点名称: $SITE_NAME"
    echo "域名: $DOMAIN"
    echo "Web 访问端口: $WEB_PORT (已分配)"
    echo "数据库访问端口: $DB_PORT (已分配)"
    echo "Redis 访问端口: $REDIS_PORT (已分配)"
    echo "Web 容器IP: $WEB_IP"
    echo "数据库容器IP: $DB_IP"
    echo "Redis 容器IP: $REDIS_IP"
...
main() {
    ...
    # 第一部分：WordPress 容器部署
    log "=== 开始 WordPress 容器部署 (融合版) ==="
    
    # 检查 Must-Use Plugins 目录
    check_mu_plugins_directory
    ...
    log "✅ WordPress 容器部署完成！"
    
    # 第二部分：宝塔反向代理和 SSL 配置
    log "=== 开始配置宝塔反向代理和 SSL（内网+FRP优化版）==="
